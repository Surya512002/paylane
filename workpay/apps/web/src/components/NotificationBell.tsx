...
type N = { id: string; title: string; body: string; href?: string | null; read: boolean };
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 shadow-lg">
          ...
          {items.length === 0 ? (
            <p className="text-xs text-[var(--muted)]">No alerts yet</p>
          ) : (
            <ul className="max-h-64 space-y-2 overflow-auto">
              {items.slice(0, 12).map((n) => (
                ...
                >
                  {n.href ? (
                    <Link href={n.href} onClick={() => setOpen(false)} className="font-semibold">
                      {n.title}
                    </Link>
                  ) : (
