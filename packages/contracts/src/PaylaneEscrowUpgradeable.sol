...
/// @title PaylaneEscrowUpgradeable — UUPS USDC escrow for Arc (Mode A)
/// @notice Upgradeable non-custodial job escrow. Owner can upgrade logic; cannot drain job balances.
contract PaylaneEscrowUpgradeable is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    /// @custom:storage-location erc7201:paylane.escrow.storage
    IERC20 public usdc;
    uint16 public platformFeeBps;
    address public feeRecipient;
    address public arbitrator;
    uint64 public defaultReviewWindow;
    ...
    event JobCreated(
        uint256 indexed jobId, bytes32 jobRef, address indexed client, address worker, uint128 amount, uint64 deadline
    );
    ...
    function initialize(address usdc_, address feeRecipient_, address arbitrator_, address owner_)
        external
        initializer
    {
        if (usdc_ == address(0) || feeRecipient_ == address(0) || arbitrator_ == address(0) || owner_ == address(0)) {
    ...
    function autoRelease(uint256 jobId) external nonReentrant whenNotPaused {
        Job storage j = jobs[jobId];
        if (j.status != JobStatus.Delivered) revert InvalidStatus();
        if (block.timestamp < uint256(j.deliveredAt) + uint256(j.reviewWindow)) revert ReviewWindowActive();
    ...
    function _refundClient(uint256 jobId, Job storage j) internal {
        usdc.safeTransfer(j.client, j.amount);
        emit FundsRefunded(jobId, j.client, j.amount);
    }
}
